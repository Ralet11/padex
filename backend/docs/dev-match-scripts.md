...
- `npm run dev:matches:create -- --count 12 --payment-required false`
  Genera partidos demo con el flujo viejo, sin cobro al unirse.
...
- `npm run dev:matches:reset -- --scope demo --count 12 --payment-required false --yes`
  Limpia demo matches y recrea una tanda nueva con el flujo viejo, sin pagos al unirse.
...
- `payment_required` ahora nace en `true` por default para partidos nuevos y demo.
- `--payment-required false` sirve si queres forzar el flujo viejo sin cobro al unirse.
